...
"""
    ...
    with right:
        if mcol not in d_year.columns:
            ...
            hover_data = {}
            # Always show some core columns if they exist
...
with tab_trends:
    ...
    with col_control2:
        selected_year = st.selectbox("Select Year", years, index=len(years) - 1, key="trend_year")
    
    # Determine which data to use based on selection
    if selected_county == "Statewide":
        # Use statewide data
        df_trend = df_state.copy()
    else:
        # Filter for specific county
        df_trend = df_map[df_map["county"] == selected_county].copy()
    
    # Filter by year if needed for single-year view
    df_trend_year = df_trend[df_trend["year"] == selected_year] if selected_year in df_trend["year"].values else pd.DataFrame()
    
    col1, col2 = st.columns(2)
    
    with col1:
        # Litter Collection & Recycling - Stacked Bar Graph
        if 'litter' in df_trend.columns:
            ...
            if 'recycled' in df_trend.columns:
                fig_trends.add_trace(go.Bar(
                    x=df_trend["year"],
                    y=df_trend["recycled"],
                    name="Recycled",
            ...
            fig_trends.update_layout(
                title=f"Litter Collection & Recycling Trends - {selected_county}",
                xaxis_title="Year",
    ...
    with col2:
        # Community Engagement Trends - Bars + Line (dual-axis)
        # Handle different possible column names for volunteer hours
        vol_col = None
        if 'vol_hours' in df_trend.columns:
            vol_col = 'vol_hours'
        elif 'volunteer_hours' in df_trend.columns:
            vol_col = 'volunteer_hours'
        
        partners_col = 'partners' if 'partners' in df_trend.columns else None
        
        if vol_col or partners_col:
            fig_vol = make_subplots(specs=[[{"secondary_y": True}]])
            
            if vol_col:
                fig_vol.add_trace(
                    go.Bar(x=df_trend["year"], y=df_trend[vol_col], 
            ...
            if partners_col:
                fig_vol.add_trace(
                    go.Scatter(x=df_trend["year"], y=df_trend[partners_col], 
    ...
    # Cumulative Impact - for Statewide only
    if selected_county == "Statewide" and 'litter' in df_state.columns:
